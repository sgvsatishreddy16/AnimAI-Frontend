export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. Voice: Google TTS - 100% FREE, US English accent. Splits long text automatically.
    const fullDialogue = dialogues.join('. ');
    const chunks = fullDialogue.match(/.{1,200}(?:\s|$)/g) || [fullDialogue]; // Google limit = 200 chars
    ...
    res.status(200).json({
      status: 'success',
      images: images,
      audio_urls: audioUrls, // Array of URLs - frontend will join them
      ratio: ar,
      cost_usd: 0
    });
